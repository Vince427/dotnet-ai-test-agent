...
coding-agent orchestrator — avoid the collision. **Status:** done. All
`Symphony*` C# types/files were renamed to `AgentLoopWorkbench*` (A5). The generated
artifact is now `docs/agentloop.html` (see `docs/github-pages.md`) and the "AgentLoop ticket"
model name intentionally evokes OpenAI's ticket workflow we reused.
